...
이렇게 하면 **OpenCode 터미널**(채팅/입력)과 **Manifest View**(상시 시각화)가 함께 실행됩니다. OpenCode 설치 및 PATH 설정이 필요합니다. OpenCode에 `manifest-orchestrator` 에이전트를 설정해 두어야 합니다 (기본 에이전트 이름은 `.manifest/settings.json` 또는 환경변수 `MANIFEST_OPENCODE_AGENT`로 변경 가능).

## Core Concepts

### UI 구성

- **OpenCode 터미널**: 채팅/입력·대화는 전부 OpenCode에서 합니다.
- **Manifest View (MVP)**: blueprint·구조·drift·태스크를 상시 표시하는 별도 창. 추후 Electron 앱으로 전환 예정.
