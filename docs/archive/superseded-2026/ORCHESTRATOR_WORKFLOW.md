...
1. **사용자 입력**: 일반 텍스트 입력 (명령어가 아닌 경우)
   - 예: "사용자 인증 기능을 구현해줘"
   - 위치: `app.py:process_command()` (line 1704)
   - 조건: `not user_input.startswith("/")` (line 1726)

2. **Orchestrator 처리**:
   - AgentManager.create_agent("orchestrator") 호출 (line 1736)
   - OrchestratorAgent 인스턴스 생성 (`manager.py:100`)
   - orchestrator_instance.coordinate() 호출 (line 1769)
   - OrchestratorAgent.coordinate() → executor.execute_agent() (line 90)
   - HookManager.apply_hooks()로 프롬프트 가로채기 (executor.py:89)
   - LLM API 호출 (스트리밍)
...
4. **Worker Squad 자동 시작** (선택적):
   ...
   - Worker Squad는 Event-driven 모드로 자동 실행 (planner → tdd_test → coder → test → ...)

**코드 위치** (정확한 라인 번호):
- Orchestrator 처리: `app.py:1726-1857`
- Task 생성: `app.py:1909-2016`
- Worker Squad 시작: `app.py:1995-2008`
...
**위치**: `command_handler.py:153-178` (_handle_start_task 메서드)
**구현 상태**: ✅ 완전 구현됨
