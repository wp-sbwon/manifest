"""Integration: layer-writer agent output must match golden snapshot.

The golden file captures the expected deterministic output. If this test fails,
either the prompt needs re-engineering or the agent contract changed — update
tests/golden/layer_writer_golden.json after verifying the new output is correct.
"""
import json
import os
import shutil
...
def test_layer_writer_output_matches_golden(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """layer-writer output matches golden (empty parent -> empty children)."""
    ...
    assert result["children"] == golden["children"], (
        "Layer-writer output does not match golden. "
        "Expected %s; got %s. "
        "Update tests/golden/layer_writer_golden.json if the new output is correct."
