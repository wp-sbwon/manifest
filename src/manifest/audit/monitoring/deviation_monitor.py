"""
Deviation Monitor: Design Plan vs Actual Code alignment.
...
class DeviationMonitor:
    """Monitor Design Plan vs Actual Code by driving CodeWatcher."""

    def __init__(
        self,
        project_root: Optional[Path] = None,
        manifest_dir: Optional[Path] = None,
        on_deviation_updated: Optional[Callable[[], None]] = None,
    ):
        ...
        self.on_deviation_updated = on_deviation_updated
        self._running = False
    ...
    def check_and_update(self) -> bool:
        ...
        detect the change and refresh Diagram/Deviation views.
        ...
        try:
            ...
            write_health_to_state(self.manifest_dir)
        except Exception as e:
            logger.debug("Project metrics update failed: %s", e)
        if updated and self.on_deviation_updated:
            try:
                self.on_deviation_updated()
            except Exception as e:
                logger.debug("DeviationMonitor on_deviation_updated callback error: %s", e)
        return updated

    def start_monitoring(self, interval_seconds: float = 10.0):
        """Start periodic deviation monitoring.
        ...
        self._running = True
        logger.info(
            "DeviationMonitor started (interval=%.1fs); use check_and_update() on a timer",
            interval_seconds,
        )
