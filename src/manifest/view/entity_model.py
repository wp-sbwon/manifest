"""
Load design and code blueprints, validate, build view schema from three sources: blueprint, code, test_results.
"""
...
def _is_test_entity(eid: str) -> bool:
    """True if entity is a test-extracted component (excluded from health stats)."""
    return eid.startswith("comp-tests.")
...
def entities_and_comp_status_from_view_schema(view_schema: Dict[str, Any]) -> tuple:
    ...
    Test-extracted components (comp-tests.*) are excluded from comp_status."""
    entities_out: List[Dict[str, Any]] = []
    comp_status: Dict[str, str] = {}
    for ve in view_schema.get("entities") or []:
        eid = ve.get("id") or ""
        val = ve.get("validation") or {}
        if not _is_test_entity(eid):
            comp_status[eid] = val.get("status") or "planned"
        single = {}
        for key in ("narrative", "governance", "blueprint", "protocol", "profile", "symbol", "traits", "topology_actual", "preview"):
            raw = ve.get(key)
            single[key] = to_single_value(raw if raw is not None else {})
        entities_out.append({
            "id": eid,
            "children": unwrap_list_field(ve, "children"),
            "dependencies": unwrap_list_field(ve, "dependencies"),
            "outgoing_contracts": unwrap_list_field(ve, "outgoing_contracts"),
            **single,
        })
...
def _comp_status_from_view(view_schema: Dict[str, Any]) -> Dict[str, str]:
    ...
    for ve in view_schema.get("entities") or []:
        eid = ve.get("id")
        if eid and not _is_test_entity(eid):
...
def _load_test_results(manifest_dir: Path) -> Dict[str, Any]:
    """Load test results by AST-parsing test files for manifest_assertion markers."""
    try:
        from manifest.audit.code.test_result_collector import collect_test_results
        project_root = Path(manifest_dir).parent
        return collect_test_results(project_root)
    except Exception:
        return {}
...
def get_entities_for_view(
    ...
    """Load design and code, build view schema. When write_view is True, write blueprint_view.json."""
    manifest_dir = Path(manifest_dir)
    ...
    test_results = _load_test_results(manifest_dir)
    view_schema = build_view_schema(design, code, test_results=test_results)
    view_write_ok = write_view_schema(manifest_dir, view_schema) if write_view else False
